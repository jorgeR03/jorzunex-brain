...
| 2026-07-29 | Sonnet 5 | **PoC-2 completada.** Postgres+pgvector confirmado arriba (`docker compose up -d` terminó el pull tras varios reintentos — la red iba lenta, no había ningún problema de configuración). Embedder local y gratuito (`gateway/src/embedder/`, `@xenova/transformers`, `Xenova/paraphrase-multilingual-MiniLM-L12-v2`, 384 dim, CPU, sin API de pago — ADR-0001 §3). Chunker por encabezados + ventanas solapadas (`gateway/src/ingest/chunker.ts`) y pipeline de ingesta standalone (`npm run ingest`, sin n8n todavía). `document_chunks` ajustada a `vector(384)` + `UNIQUE(source_path, chunk_index)` para upsert idempotente (antes vector(1536) sin usar). `Retriever` sobre pgvector (`gateway/src/retriever/`, similaridad coseno vía `<=>`, top-k) — ningún consumidor habla con Postgres directamente. `askBrain()` ahora recupera primero el top-k del Retriever y lo pasa como contexto principal (además de mantener Read/Glob/Grep como respaldo); nuevas flags `--no-rag`/`--top-k` en la CLI. Ingestados 24 archivos Markdown (`docs/`+`prompts/`, incluye lo añadido por la sesión paralela de wiki/Obsidian) → 184 chunks. Prueba real: 14 preguntas de oro sobre `docs/investigacion/`+`prompts/`+wiki, todas con RAG activo (6 fragmentos c/u) y citando el archivo fuente correcto — 12 con `claude-sonnet-5`, 2 con `claude-haiku-4-5` (`--task classify`), nunca Opus/Fable. Latencia de retrieval (embedding de la consulta + pgvector) medida en ~5-7ms — muy por debajo del objetivo p95<1s del roadmap. Coste real de las pruebas de PoC-2: ~$1.00 (16 interacciones, vía cuota de suscripción de Claude Code, sin API de pago). Nota: durante esta sesión otra sesión trabajó en paralelo sobre el mismo repo (wiki + adopción de Obsidian en ADR-0001 + fix de `CLAUDE.md`) — no se tocó ni revirtió nada de ese trabajo, solo se re-ingestó para incluirlo. | F2: n8n en producción (3–5 flujos reales) + primer Managed Agents scheduled deployment (informe semanal GitHub+Linear+wiki) según `docs/investigacion/05-roadmap-pocs-backlog.md`, o alternativamente Motor comercial v0 (`docs/VISION.md` pilar 3) si Jorge prioriza ingresos sobre automatización — pedir preferencia a Jorge antes de arrancar. Pendiente menor: si el corpus de `docs/wiki/` crece mucho, considerar índice ANN (ivfflat) en `document_chunks` (hoy sequential scan, de sobra para 184 chunks). |
| 2026-07-29 | Sonnet 5 | Creado y probado `.mcp.json` en la raíz (paso `3.1.4`), en paralelo a otra sesión que trabajaba en `gateway/`/`ingest/`/`infra/` (no se tocó nada de eso, solo se consultó `docker ps` para confirmar que Postgres seguía arriba). 3 servidores MCP compartidos del proyecto: `github` (remoto oficial `https://api.githubcopilot.com/mcp/`, Streamable HTTP, header `Authorization: Bearer ${GITHUB_TOKEN}` — el paquete npm `@modelcontextprotocol/server-github` está deprecado/archivado, por eso el remoto), `postgres` (`npx @modelcontextprotocol/server-postgres`, stdio, `${DATABASE_URL:-postgresql://brain:brain_dev_password@localhost:5432/jorzunex_brain}` — este paquete también está deprecado/archivado pero sigue funcionando y no hay sucesor oficial), `filesystem` (`npx @modelcontextprotocol/server-filesystem`, acotado a `docs/` y `prompts/`). Verificación real (no solo `npm view`): handshake MCP completo (`initialize` + `tools/list`) por stdio contra `postgres` (conectó de verdad al contenedor `jorzunex-brain-postgres` sano y expuso su tool `query` de solo lectura) y contra `filesystem` (listó sus 14 tools, incluida `list_allowed_directories`); `curl` contra el endpoint remoto de `github` confirmó protocolo MCP + `401` esperado sin token; `claude mcp list` confirmó el JSON válido, los 3 servidores detectados y el aviso correcto de `GITHUB_TOKEN` ausente. Aviso documentado en `docs/mcp-setup.md`: el `filesystem` server no es de solo lectura de verdad (tiene `write_file`/`edit_file`/`move_file`), solo está acotado por directorio — no se tocaron permisos de Claude Code porque no se pidió explícitamente. No se pudo completar la aprobación interactiva de "confiar en el workspace" (el harness de esta sesión no tiene TTY) — queda pendiente para la primera sesión interactiva de Jorge. Documentación completa en `docs/mcp-setup.md` (qué es cada servidor, variables de entorno, cómo probarlo con `/mcp`/`claude mcp list`, qué falta). | Falta que Jorge: (1) cree un GitHub PAT y exporte `GITHUB_TOKEN` en su shell antes de abrir Claude Code; (2) abra `claude` en el repo y acepte el diálogo de confianza del workspace para que los 3 servidores pasen de `⏸ Pending approval` a `✔ Connected`. Después, seguir con F2 (n8n/Managed Agents) o Motor comercial v0 según decida Jorge (ver fila anterior). |
| 2026-07-29 | Sonnet 5 | Rellenada la wiki (`docs/wiki/`) con información real extraída del Google Drive de Jorge (`Hoja de vida/` y `JorZunex Solutions/`, con exclusión explícita de `Balance de la empresa/` y `Documentación legal de la empresa/`) y de fuentes públicas (GitHub, web corporativa). `equipo/jorge.md`: formación (Ing. Sistemas UCO 2020–act., Técnico SENA), experiencia (Casa Inmobiliaria 2019–2025), stack técnico completo y redes (GitHub `jorgeR03`, Instagram `@jorge_andre_ss`) desde el CV (`HV_Jorge_Andres_Carmona_2026_v2.pdf`) — se omitieron C.C., teléfono y contactos de referencias personales. Creado `equipo/jose.md` para el cofundador **José Manuel Zuluaga Figueroa** ("el Zulu", GitHub `elzulu`), encontrado como coautor del trabajo de grado Vitalora Care y como segundo miembro del equipo en la web de JorZunex. `empresa/jorzunex.md`: secciones "Qué hacemos", "Canales" y "Modelo de ingresos" rellenadas con datos reales de la web corporativa (`jorzunex.github.io`) y los README de cada producto (precios reales COP/EUR). Creadas 11 fichas reales en `docs/wiki/proyectos/` (GymApp/PowerFit, RepartOS, MotoGest Pro, Vitalora Care, JorZunex Inmobiliaria, PharmaPOS, Zentrax, Agende de Ventas, JorZunexFinanzas, página web, Notion_2.0/Notely — este último de estado incierto, posible proyecto de práctica) cruzando el Drive con los 10 repos de `C:\Users\jorge\ProyectosJorZunex\`; borrado `_ejemplo-proyecto.md`. **Hallazgo clave para el pilar 3 de VISION.md:** el proyecto `Agende de Ventas` ya es un motor de prospección funcional (mini-CRM + generación de mensajes WhatsApp/email para los 5 SaaS de la empresa) — revisar si "Motor comercial v0" debe integrarse con él en vez de duplicarlo. Datos omitidos a propósito por sensibilidad: contraseñas/API keys de ejemplo en READMEs de `JorZunex Inmobiliaria` y `MotoGest-pro` (se mencionó que existen credenciales demo, sin copiar los valores), y el contenido de `Copia de Gastos e Ingresos.xlsx` (JorZunexFinanzas, datos financieros reales, no abierto). LinkedIn de Jorge y de José no se pudieron verificar por bloqueo anti-bot (HTTP 999) al hacer `WebFetch`. Nota operativa: un commit de esta sesión (proyectos de wiki) y uno de una sesión paralela (Langfuse en `gateway/`) se mezclaron en un único commit (`414cef1`) por trabajar ambas sesiones sobre el mismo working tree sin worktrees separados — el contenido de ambas quedó íntegro, solo el mensaje de commit no refleja el trabajo de wiki. | Pendiente que Jorge confirme/aclare: (1) si `JorZunex Inmobiliaria` es la evolución de "EVO Digital" (CV/GitHub) o un proyecto distinto; (2) el propósito real de `Notion_2.0`/Notely (¿práctica o producto?); (3) revisar y sincronizar `docs/wiki/proyectos/agende-de-ventas.md` con `docs/VISION.md` pilar 3 antes de construir nada nuevo de "motor comercial". Sesiones futuras: considerar separar sesiones paralelas en worktrees de git para evitar que se mezclen commits de trabajo no relacionado. |
| 2026-08-01 | Sonnet 5 | **ADR-0003 (Atlas unificado) implementado en gran parte.** (1) Orquestador (`gateway/src/orchestrator.ts`, `POST /atlas`): clasifica ask/dev_task/commercial con Haiku, nunca autoriza deploy, cola de trabajos async en memoria (`devJobs.ts`, campo `kind`) para no bloquear la conversación en tareas largas, memoria compartida real vía logs Markdown (`docs/wiki/proyectos/_actividad-dev/` y `_actividad-comercial/`) reingestados solos por el watcher. (2) STT local con Whisper (`gateway/src/stt/`, `@xenova/transformers`, `POST /stt`) como motor principal, navegador+corrección por IA (`resolveTranscript`) como respaldo; barge-in afinado (umbral 7 caracteres + 2 confirmaciones consecutivas) para no dispararse con ruido ambiente. (3) ElevenLabs TTS con reintentos en fallos transitorios — **cuota gratuita mensual agotada** (0/10000 caracteres, HTTP 401 `quota_exceeded`), desactivado a propósito en `gateway/.env` (comentado) para caída instantánea a voz de navegador sin llamadas de red perdidas; reactivar cuando se reinicie la cuota o se suba de plan. (4) **Motor comercial v0** (`docs/VISION.md` pilar 3): `leadsClient.ts` + `commercialAgent.ts`, integrado con el CRM ya existente "Agende de Ventas" (no duplicado) vía HTTP determinista — el modelo nunca tiene una tool de CRM ni de envío, solo WebSearch/WebFetch de solo lectura (primera habilitación de Web* en el código) y devuelve un JSON que el propio orquestador aplica (`notes`/`nextFollowUp` únicamente, nunca contacto/status). Verificado 2 veces con un lead real (Drogiray, Ibagué); un bug real (`canUseTool` faltante bloqueaba WebSearch en silencio) se detectó y corrigió en la propia sesión. **Nota de seguridad importante:** el repo de GitHub es público — por eso el contexto del motor comercial excluye a propósito teléfono/email de los leads. (5) `docs/wiki/proyectos/*.md` ya tenían indexación profunda de los 9 proyectos reales (82 archivos/2030 chunks, hecha en una sesión previa — se confirmó y refrescó, no se reconstruyó). (6) Primer trabajo programado (pilar 2 "Jarvis proactivo"): `gateway/src/scheduled/weeklyBriefing.ts`, corrido en vivo con éxito — genera resumen + sugerencias reales a partir de git log + logs de actividad + antigüedad de wiki, sin Managed Agents (decisión de Jorge: Task Scheduler de Windows, ver `gateway/src/scheduled/README.md` para el `schtasks` exacto, aún no registrado). (7) WebSearch/WebFetch habilitado también en `askBrain` (`gateway/src/gateway.ts`) como último respaldo cuando el RAG no alcanza, siempre marcado como fuente externa no confiable. Todo commiteado y pusheado (`751b25e` y el commit de este trabajo). | Pendiente, en orden de impacto: (a) decidir remuneración/plan de ElevenLabs o esperar reset de cuota; (b) registrar el `schtasks` del informe semanal (paso manual, ver README); (c) commit/push del cambio pendiente en el README de Zentrax (detectado por el propio informe semanal); (d) generalizar el guardarraíl de "acción sensible" más allá de deploy (ADR-0003 punto 3) — de momento no hay nada más que lo necesite, evitar construirlo antes de tiempo; (e) bloqueado en Jorge: espacio de trabajo de José (falta indexar sus datos), canales Slack/WhatsApp (necesitan cuentas/tokens que Jorge debe crear), decisión de IoT (backlog sin fecha), despliegue real a VPS (paso 7 de ADR-0003, sigue al final a propósito). |
